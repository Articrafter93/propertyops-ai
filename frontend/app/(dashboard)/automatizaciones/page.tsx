...
export default async function AutomatizacionesPage() {
  const [automation_runs, error_log] = await Promise.all([getAutomationRuns(), getErrorLogs()]);
      ...
      <main className="flex-1 overflow-y-auto p-6 bg-background space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card className="p-4 shadow-none border border-border flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Ejecuciones (7d)</p>
            </div>
          </Card>
          <Card className="p-4 shadow-none border border-border flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Tasa de éxito media</p>
            </div>
          </Card>
          <Card className="p-4 shadow-none border border-border flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Errores (7d)</p>
            </div>
          </Card>
        </div>
        ...
        <Card className="shadow-none border border-border overflow-hidden">
          <div className="px-5 py-3 border-b border-border">
            <h3 className="text-sm font-semibold text-foreground">Escenarios Make</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/40 border-b border-border">
                  <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground">Escenario</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">Estado</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">Última ejecución</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">Próxima</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground w-40">Tasa de éxito</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">Tiempo medio</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground">Ejecuciones (7d)</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={run.scenario_id} className="border-b border-border last:border-0 hover:bg-muted/20">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-foreground">{run.scenario}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        {formatRelativeTime(run.last_run)}
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-xs text-muted-foreground">
                      {run.next_run ? formatRelativeTime(run.next_run) : <span className="text-muted-foreground/50">—</span>}
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1.5 text-xs">
                        ...
                        {run.errors_7d > 0 && (
                          <span className="text-red-600">({run.errors_7d} error{run.errors_7d > 1 ? "es" : ""})</span>
                        )}
                      </div>
                    </td>
                  </tr>
              ...
              <div key={err.id} className="px-5 py-4 hover:bg-muted/20">
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-foreground">{err.scenario}</span>
                  </div>
                  <span className="text-xs text-muted-foreground shrink-0">{formatRelativeTime(err.timestamp)}</span>
                </div>
                <p className="text-xs text-red-700 font-medium mb-1">{err.error}</p>
                <p className="text-xs text-muted-foreground">Módulo: <code className="font-mono bg-muted px-1 rounded">{err.module}</code></p>
                <p className="text-xs text-muted-foreground mt-0.5">Payload: {err.payload_summary}</p>
              </div>
            ))}
          </div>
        </Card>
